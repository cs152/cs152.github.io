/* Copyright 2016 Google Inc. All Rights Reserved.
   Modifications copyright (C) 2023 Gabriel Hope.
...
function makeGUI() {
  ...
  let reg1dDataThumbnails = d3.selectAll("canvas[data-reg1dDataset]");
  reg1dDataThumbnails.on("click", function() {
    let newDataset = reg1dDatasets[this.dataset.reg1ddataset];
    if (newDataset === state.reg1dDataset) {
      return; // No-op.
    }
    state.reg1dDataset =  newDataset;
    reg1dDataThumbnails.classed("selected", false);
    d3.select(this).classed("selected", true);
    generateData();
    parametersChanged = true;
    reset();
  ...
  let reg1dDatasetKey = getKeyFromValue(reg1dDatasets, state.reg1dDataset);
  // Select the dataset according to the current state.
  d3.select(`canvas[data-reg1dDataset=${reg1dDatasetKey}]`)
    .classed("selected", true);
  ...
  let initializerDropdown = d3.select("#initializers").on("change", function() {
    state.initializer = initializers[this.value];
    parametersChanged = true;
    reset();
  });
  initializerDropdown.property("value",
      getKeyFromValue(initializers, state.initializer));


  let dropoutDropdown = d3.select("#dropout").on("change", function() {
    state.dropout = +this.value;
    parametersChanged = true;
    reset();
  });
  dropoutDropdown.property("value", state.dropout);
  ...
  let modeDropdown = d3.select("#mode").on("change", function() {
  ...
  modeDropdown.property("value", state.mode);
...
function drawNode(cx: number, cy: number, nodeId: string, isInput: boolean,
  ...
  let div = d3.select("#network").insert("div", ":first-child")
    ...
    .on("mouseenter", function() {
      ...
      heatMap.updateBackground(boundary[nodeId], state.discretize, state.problem === Problem.REGRESSION1D);
    })
    .on("mouseleave", function() {
      selectedNodeId = null;
      ...
      heatMap.updateBackground(boundary[nn.getOutputNode(network).id],
          state.discretize, state.problem === Problem.REGRESSION1D);
    });
  if (isInput) {
...
function updateDecisionBoundary(network: nn.Node[][], firstTime: boolean) {
  ...
  for (i = 0; i < DENSITY; i++) {
    ...
    if (state.problem === Problem.REGRESSION1D){
      let x = xScale(i);
      let input = constructInput(x, x);
      nn.forwardProp(network, input, state.mode);
      nn.forEachNode(network, true, node => {
        boundary[node.id][i][0] = x;
        boundary[node.id][i][1] = Math.max(Math.min(node.output, xDomain[1]), xDomain[0]);
      });
      if (firstTime) {
        // Go through all predefined inputs.
        for (let nodeId in INPUTS) {
          boundary[nodeId][i][0] = x;
          boundary[nodeId][i][1] = INPUTS[nodeId].f(x, x);
        }
      }
    }
    else {
...
function getLoss(network: nn.Node[][], dataPoints: Example2D[]): number {
  ...
  for (let i = 0; i < dataPoints.length; i++) {
    ...
    let output = nn.forwardProp(network, input, state.mode);
    let label = state.problem === Problem.REGRESSION1D ? dataPoint.y : dataPoint.label;
...
function updateUI(firstStep = false) {
  ...
  heatMap.updateBackground(boundary[selectedId], state.discretize, state.problem === Problem.REGRESSION1D);

  // Update all decision boundaries.
  if (state.problem === Problem.REGRESSION1D) {
    d3.select("#network").selectAll("div.canvas")
        .each(function(data: {heatmap: HeatMap, id: string}) {
      data.heatmap.updateBackground(boundary[data.id],
          state.discretize, state.problem === Problem.REGRESSION1D);
    });
  }
  else {
...
function oneStep(): void {
  ...
  trainData.forEach((point, i) => {
    ...
    if ((i + 1) % state.batchSize === 0) {
      nn.updateWeights(network, state.learningRate, state.regularizationRate, state.mode);
    }
  });
  // Compute the loss.
...
function reset(onStartup=false) {
  ...
  let outputActivation = (state.problem === Problem.CLASSIFICATION) ?
       nn.Activations.TANH : nn.Activations.LINEAR;
  network = nn.buildNetwork(shape, state.activation, outputActivation,
      state.regularization, state.dropout, constructInputIds(), state.initializer);
  lossTrain = getLoss(network, trainData);
  lossTest = getLoss(network, testData);
  drawNetwork(network);
  updateUI(true);
...
function drawDatasetThumbnails() {
  ...
  if (state.problem === Problem.REGRESSION1D) {
    for (let reg1dDataset in reg1dDatasets) {
      let canvas: any =
          document.querySelector(`canvas[data-reg1dDataset=${reg1dDataset}]`);
      let dataGenerator = reg1dDatasets[reg1dDataset];
      renderThumbnail(canvas, dataGenerator);
    }
  }
}
...
function generateData(firstTime = false) {
  ...
  generator = state.problem === Problem.REGRESSION1D ?
      state.reg1dDataset : generator;
