...
```{ojs}
//| echo : false
viewof inputshift = Inputs.range([-3, 3], {value: 0., step: 0.01, label: "  Shift"})
viewof inputscale = Inputs.range([0.01, 3], {value: 1., step: 0.01, label: "  Scale"})
...
x, y = df[:, :1], df[:, 1:]
x = Tensor((x - x.mean() + float(${inputshift})) / x.std() * float(${inputscale}))
y = Tensor((y - y.mean() + float(${inputshift})) / y.std() * float(${inputscale}))
